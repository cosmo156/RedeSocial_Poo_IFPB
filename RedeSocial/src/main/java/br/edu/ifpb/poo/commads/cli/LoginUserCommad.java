...
public class LoginUserCommad implements Commad {
    @Override
    public void execute() {
        ...
        if (admUsuarioService.userExists(nome)){
            Usuario user = admUsuarioService.logarUsuario(nome, senha);
            new CommandExecutor().executeCommad(new MenuUserCommad(user));
        }else {
